...
def init_ground(grizY_30s_ground_depths):
    ...
    for filt in "grizY":
        ground_filt_fns[filt] = file_to_fn(wfirst_path + "/scripts/pixel-level/input/LSST_" + filt + ".txt")
    ...
    for filt in ["Z087", "Y106", "J129", "H158", "F184", "K193"]:
        WFI_filt_fns[filt] = file_to_fn(wfirst_path + "/scripts/pixel-level/input/" + filt + ".txt")
...
slew_fn = file_to_fn(wfirst_path + "/scripts/pixel-level/input/slewtable_I32000_t0.40_vmax0.12_Tr2.0.txt", col = 2)
...
def make_SNe(square_degrees, cadence, survey_duration, rates_fn, redshift_set, IFS_trigger_params, tier_filters, tier_exptimes, ground_depths, dithers, parallel_filters,
    ...
    source = sncosmo.SALT2Source(modeldir=wfirst_path + "/salt2_extended/")
...
slew_fn = file_to_fn(wfirst_path + "/scripts/pixel-level/input/" + survey_parameters["slew_table"], col = 2)


    
WFI_args = {"PSFs": PSFs_WFC, "source_dir": wfirst_path + "/scripts/pixel-level/input",
            ...
            "zodi_fl": file_to_fn(wfirst_path + "/scripts/pixel-level/input/" + survey_parameters["zodiacal_background"]),
            "bad_pixel_rate": survey_parameters["bad_pixel_rate"],
            "waves": arange(6000., 22500.1, 25.)}

IFS_args = {"PSFs": PSFs, "source_dir": wfirst_path + "/scripts/pixel-level/input",
            ...
            "IFURfl": file_to_fn(wfirst_path + "/scripts/pixel-level/input/" + survey_parameters["IFU_resolution"]),
            "zodifl": file_to_fn(wfirst_path + "/scripts/pixel-level/input/" + survey_parameters["zodiacal_background"]),
            "effareafl": file_to_fn(wfirst_path + "/scripts/pixel-level/input/" + survey_parameters["IFU_effective_area"]),
...
rates_fn_full = file_to_fn(wfirst_path + "/scripts/pixel-level/input/" + survey_parameters["SN_rates"]) # per 1e-4 year Mpc^3 (h = 0.7)
